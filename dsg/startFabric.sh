...
if [ "$CC_SRC_LANGUAGE" = "go" -o "$CC_SRC_LANGUAGE" = "golang"  ]; then
	...
	pushd ../chaincode/dsg/go
	GO111MODULE=on go mod vendor
	popd
	echo Finished vendoring Go dependencies
elif [ "$CC_SRC_LANGUAGE" = "java" ]; then
	CC_RUNTIME_LANGUAGE=java
	CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/fabric-samples/chaincode/dsg/java/build/install/dsg
  echo Compiling Java code ...
  pushd ../chaincode/dsg/java
  ./gradlew installDist
  popd
  echo Finished compiling Java code
elif [ "$CC_SRC_LANGUAGE" = "javascript" ]; then
	CC_RUNTIME_LANGUAGE=node # chaincode runtime language is node.js
	CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/fabric-samples/chaincode/dsg/javascript
elif [ "$CC_SRC_LANGUAGE" = "typescript" ]; then
	CC_RUNTIME_LANGUAGE=node # chaincode runtime language is node.js
	CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/fabric-samples/chaincode/dsg/typescript
...
REGEX='Package ID: (.*), Label: dsgv1'
if [[ `${PEER0_ORG1} lifecycle chaincode queryinstalled` =~ $REGEX ]]; then
  PACKAGE_ID_ORG1=${BASH_REMATCH[1]}
else
  echo Could not find package ID for dsgv1 chaincode on peer0.org1.example.com
  exit 1
fi
...
${PEER0_ORG2} lifecycle chaincode install dsg.tar.gz

echo "Installing smart contract on peer1.org2.example.com"
${PEER1_ORG2} lifecycle chaincode install dsg.tar.gz
...
if [[ `${PEER0_ORG2} lifecycle chaincode queryinstalled` =~ $REGEX ]]; then
  ...
  echo Could not find package ID for dsgv1 chaincode on peer0.org2.example.com
  exit 1
fi
...
Next, use the dsg applications to interact with the deployed dsg contract.
The dsg applications are available in multiple programming languages.
