...
for LANGUAGE in ${LANGUAGES}; do
    ...
    if [ -d ${LANGUAGE} ]; then
        ...
        if [ ${LANGUAGE} = "javascript" -o ${LANGUAGE} = "typescript" ]; then
            ...
            copy_logs $? dsg-${LANGUAGE}-enrollAdmin
            ${COMMAND} ${PREFIX}registerUser${SUFFIX}
            copy_logs $? dsg-${LANGUAGE}-registerUser
            ${COMMAND} ${PREFIX}query${SUFFIX}
            copy_logs $? dsg-${LANGUAGE}-query
            ${COMMAND} ${PREFIX}invoke${SUFFIX}
            copy_logs $? dsg-${LANGUAGE}-invoke
    ...
    echo -e "\033[32m finished dsg test (${LANGUAGE})" "\033[0m"
done
